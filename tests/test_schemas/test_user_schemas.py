...
def test_user_create_valid(user_create_data):
    # Test when nickname is provided
    user_create_data['nickname'] = 'johnny_doe'  # Add nickname to the data
    user = UserCreate(**user_create_data)
    assert user.nickname == 'johnny_doe'
    assert user.password == user_create_data["password"]

    # Test when nickname is not provided (should be None by default)
    user_create_data.pop('nickname', None)  # Remove nickname to test the default None value
    user = UserCreate(**user_create_data)
    assert user.nickname is None  # Should be None if not provided
    assert user.password == user_create_data["password"]
...
def test_login_request_valid(login_request_data):
    ...
    login_request_data['email'] = 'john.doe@example.com'  # Add a valid email
